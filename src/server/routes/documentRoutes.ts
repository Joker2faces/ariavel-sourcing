...
export function createPortalDocumentRouter(documentService: DocumentService): Router {
  const router = Router();
  ...
  router.get('/invitations/:invitationId/quote-template', async (req: Request, res: Response) => {
    try {
      ...
      const tenantId = (req as Request & { portalTenantId?: string }).portalTenantId;
  ...
  router.post('/invitations/:invitationId/quote-import', async (req: Request, res: Response) => {
    try {
