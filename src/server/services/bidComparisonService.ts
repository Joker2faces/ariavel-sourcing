...
import type {
  ...
  NormalizedQuoteLine,
  LineBestPrice,
  CommercialTermsComparison,
  EvaluationCriterion,
  SupplierEvaluationScore,
  ExchangeRateSnapshot,
  BidLineException,
} from '../../shared/types/bid.js';
...
export class BidComparisonError extends Error {
  constructor(message: string) { super(message); this.name = 'BidComparisonError'; }
}
...
export interface BidComparisonService {
  ...
  getLatestSnapshot(tenantId: string, eventId: string): Promise<ComparisonSnapshot | null>;
  getSnapshot(tenantId: string, snapshotId: string): Promise<ComparisonSnapshot | null>;
  listSnapshots(tenantId: string, eventId: string): Promise<ComparisonSnapshot[]>;

  setManualTechnicalScore(
    tenantId: string,
    snapshotId: string,
    supplierId: string,
    score: number,
...
function convertToBase(amount: number, currency: string, baseCurrency: string, rates: Record<string, number>): number | undefined {
  if (currency === baseCurrency) return amount;
  const baseRate = rates[baseCurrency];
  const fromRate = rates[currency];
  if (baseRate == null || fromRate == null) return undefined;
...
function allocateFreight(
  ...
  freightCurrency: string | undefined,
  freightIncluded: boolean | undefined,
  lines: NormalizedQuoteLine[],
  baseCurrency: string,
  rates: Record<string, number>,
  policy: 'PROPORTIONAL_TO_LINE_VALUE' | 'EQUAL_PER_LINE' | 'MANUAL',
): Map<string, number> {
  const allocations = new Map<string, number>();
  if (!freightTotal || freightIncluded) return allocations;
  ...
  if (bidLines.length === 0) return allocations;
...
function normalizeQuoteLine(
  eventLine: SourcingLine,
  quoteLine: { unitPrice?: number; currency?: string; leadTimeDays?: number; moq?: number; notes?: string } | undefined,
  baseCurrency: string,
  fxRates: Record<string, number>,
): NormalizedQuoteLine {
  const exceptions: BidLineException[] = [];
  ...
  if (eventLine.requestedDeliveryDate && quoteLine.leadTimeDays != null) {
    ...
    if (approxDelivery > deadline) exceptions.push('LATE_DELIVERY');
  }
...
function normalizeQuote(
  ...
  quote: SupplierQuote | undefined,
  eventLines: SourcingLine[],
  baseCurrency: string,
  fxRates: Record<string, number>,
  freightPolicy: 'PROPORTIONAL_TO_LINE_VALUE' | 'EQUAL_PER_LINE' | 'MANUAL',
): NormalizedQuote {
  if (!quote || quote.status !== 'SUBMITTED') {
    const lines: NormalizedQuoteLine[] = eventLines.map(el => ({
      lineId: el.id,
      lineDescription: el.description,
      requestedQuantity: el.quantity,
      requestedUnit: el.unit,
      isNoBid: true,
      exceptions: ['NO_BID' as BidLineException],
    }));
    return {
  ...
  // Freight allocation
  const freightAllocations = allocateFreight(
    quote.freightTotal,
    quote.freightCurrency,
    quote.freightIncluded,
    lines,
    baseCurrency,
    fxRates,
    freightPolicy,
  ...
  const allExceptions = [...new Set(lines.flatMap(l => l.exceptions))] as BidLineException[];
  if (!quote.commercialTerms || !quote.paymentTerms) allExceptions.push('MISSING_COMMERCIAL_TERMS');
  if (quote.validityDays != null && quote.validityDays < 14) allExceptions.push('EXPIRED_QUOTE');
...
function calcLineBestPrices(normalizedQuotes: NormalizedQuote[], eventLines: SourcingLine[]): LineBestPrice[] {
...
function buildCommercialComparisons(
  ...
  return invitations.map(inv => {
    ...
    const exceptions: BidLineException[] = [];
    if (!quote?.commercialTerms || !quote?.paymentTerms) exceptions.push('MISSING_COMMERCIAL_TERMS');
    return {
...
function scoreSuppliers(
  ...
  const definedLt = avgLeadTimes.filter((v): v is number => v != null);
...
export function createBidComparisonService(
  ...
  return {
    async buildSnapshot(tenantId, eventId, eventLines, input, userId, now) {
      ...
      if (!fxRates[input.baseCurrency]) fxRates[input.baseCurrency] = 1;

      const exchangeRateSnapshot: ExchangeRateSnapshot | undefined = Object.keys(fxRates).length > 1 ? {
      ...
      const normalizedQuotes: NormalizedQuote[] = invitations.map(inv => {
        const quote = quotes.find(q => q.invitationId === inv.id);
