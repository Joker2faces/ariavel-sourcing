...
export class ExtractionNotEnabledError extends Error { constructor() { super('Document AI extraction is not enabled'); } }
...
export function validateMimeType(filename: string, declaredMime: string, firstBytes?: Buffer): void {
...
export interface DocumentService {
  initiateUpload(tenantId: string, input: AttachmentInput, userId: string, now: string): Promise<PresignedUploadResponse>;
  confirmUpload(tenantId: string, attachmentId: string): Promise<Attachment>;
...
export function createDocumentService(
  ...
  function genObjectKey(tenantId: string, entityType: string, entityId: string, filename: string): string {
  ...
  return {
    ...
    async extractDocument(tenantId, attachmentId, provider) {
      if (!provider.isEnabled) throw new ExtractionNotEnabledError();
      ...
      return {
        status: result.confidence >= 0.5 ? 'VALID' as const : 'ERROR' as const,
