...
export function SupplierSourceDrawer({ service, onClose, onSaved }: { service: SupplierService; onClose: () => void; onSaved: () => void }) {
  ...
  const [selectedBoard, setSelectedBoard] = useState<MondayBoardDescriptor | undefined>(undefined);
  ...
  // listBoards() returns lightweight descriptors with columns: [] — real
  // column metadata (including the synthesized Item / Name column) only
  // comes from getBoard(), so the detailed board must be fetched after the
  // buyer picks one, not read off the list response.
  useEffect(() => {
    if (!boardId) { setSelectedBoard(undefined); return; }
    let cancelled = false;
    setLoadingBoard(true);
    service.getBoard(boardId).then(detailed => { if (!cancelled) setSelectedBoard(detailed); }).finally(() => { if (!cancelled) setLoadingBoard(false); });
  ...
  return <Drawer title="Configure supplier source" onClose={onClose}>
    ...
    {mode === 'MONDAY_BOARD' ? <div className="mapping-workflow"><label className="field" htmlFor="supplier-board"><span>Supplier board</span><select id="supplier-board" value={boardId} onChange={event => { setBoardId(event.target.value); setMappings([]); }}><option value="">Choose a board</option>{boards.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      {loadingBoard ? <p>Loading board columns…</p> : null}
