...
export function SupplierSourceDrawer({ service, onClose, onSaved }: { service: SupplierService; onClose: () => void; onSaved: () => void }) {
  ...
  return <Drawer title="Configure supplier source" onClose={onClose}>
    ...
    {mode === 'MONDAY_BOARD' ? <div className="mapping-workflow"><label className="field" htmlFor="supplier-board"><span>Supplier board</span><select id="supplier-board" value={boardId} onChange={event => { setBoardId(event.target.value); setMappings([]); }}><option value="">Choose a board</option>{boards.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      ...
      {preview.length ? <section className="mapping-preview"><h3>Preview mapped suppliers</h3><p>Fictional sample data from the selected development provider.</p><div className="preview-list">{preview.map((item, index) => <div key={String(item.mondayItemId ?? index)}><strong>{String(item.name ?? 'Unnamed supplier')}</strong><span>{[item.category, item.country, item.email].filter(Boolean).map(String).join(' · ') || 'Only required fields mapped'}</span></div>)}</div></section> : null}</> : null}</div> : <div className="source-note"><strong>Ariavel will manage supplier records.</strong><p>Records remain behind the tenant-scoped repository and can move to Document DB in a future milestone.</p></div>}
