...
export class InvitationNotFoundError extends Error {
    constructor() { super('Invitation not found'); }
}
export class InvitationInvalidStatusError extends Error {
    constructor(msg) { super(msg); }
}
export function createInvitationService(invRepo, auditRepo) {
    ...
    return {
        ...
        async open(rawToken) {
            ...
            if (invitation.status === 'REVOKED')
                throw new InvitationInvalidStatusError('Invitation has been revoked');
        ...
        async revoke(tenantId, id, revokedByUserId) {
            ...
            if (invitation.status === 'SUBMITTED')
                throw new InvitationInvalidStatusError('Cannot revoke a submitted invitation');
            if (invitation.status === 'REVOKED')
                throw new InvitationInvalidStatusError('Already revoked');
            const n = now();
        ...
        async regenerate(tenantId, id, userId) {
            ...
            if (!invitation)
                throw new InvitationNotFoundError();
            if (invitation.status === 'SUBMITTED')
                throw new InvitationInvalidStatusError('Cannot regenerate token for submitted invitation');
            if (invitation.status === 'REVOKED')
                throw new InvitationInvalidStatusError('Cannot regenerate token for revoked invitation');
            const rawToken = generateRawToken();
            const tokenHash = hashToken(rawToken);
            const n = now();
