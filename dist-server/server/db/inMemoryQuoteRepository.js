...
export function createInMemoryQuoteRepository(seed = []) {
    const store = new Map(seed.map(q => [q.id, { ...q }]));
    function genId() { return randomBytes(12).toString('hex'); }
    function byInvitation(tenantId, invitationId) {
    ...
    return {
        async upsertDraft(tenantId, invitationId, eventId, supplierId, supplierNameSnapshot, input, now) {
            ...
            const doc = { id: genId(), tenantId, invitationId, eventId, supplierId, supplierNameSnapshot, status: 'DRAFT', lines: input.lines, commercialTerms: input.commercialTerms, paymentTerms: input.paymentTerms, validityDays: input.validityDays, supplierNotes: input.supplierNotes, version: 1, createdAt: now, updatedAt: now };
        ...
        async submit(tenantId, invitationId, now) {
            ...
            if (!existing || existing.status !== 'DRAFT')
                return null;
            const updated = { ...existing, status: 'SUBMITTED', submittedAt: now, updatedAt: now, version: existing.version + 1 };
            store.set(existing.id, updated);
            return { ...updated };
        },
