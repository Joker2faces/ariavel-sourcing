...
export function createInMemoryInvitationRepository(seed = []) {
    const store = new Map(seed.map(i => [i.id, { ...i }]));
    const byHash = new Map(seed.map(i => [i.tokenHash, i.id]));
    ...
    return {
        async create(tenantId, input, tokenHash, createdByUserId, now) {
            ...
            const doc = { ...input, id: genId(), tenantId, tokenHash, status: 'CREATED', createdAt: now, updatedAt: now, createdByUserId };
        ...
        async updateStatus(tenantId, id, status, extra, now) {
            ...
            if (!doc || doc.tenantId !== tenantId)
                return null;
            const updated = { ...doc, ...extra, status, updatedAt: now };
        ...
        async replaceToken(tenantId, id, newTokenHash, userId, now) {
            ...
            const updated = { ...doc, tokenHash: newTokenHash, regeneratedAt: now, regeneratedByUserId: userId, updatedAt: now };
            store.set(id, updated);
