...
export function generateQuoteTemplateCsv(rfqReference, invitationId, eventLines) {
    const metaRow = `# RFQ: ${rfqReference}`;
    const invRow = `# invitation_id: ${invitationId}`;
    const headers = TEMPLATE_HEADERS.map(csvEscape).join(',');
    const dataRows = eventLines.map(l => [l.id, l.sku ?? '', l.description, l.quantity, l.unit, '', '', '', '', '']
        .map(csvEscape).join(','));
    return [metaRow, invRow, headers, ...dataRows].join('\n');
}
export function parseQuoteImportCsv(csvContent, validLineIds, rfqReference, invitationId) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('#')) {
            ...
            if (rfqMatch)
                foundRfqRef = rfqMatch[1].trim();
            const invMatch = line.match(/^#\s*invitation_id:\s*(.+)$/);
            if (invMatch)
                foundInvId = invMatch[1].trim();
            continue;
        }
    ...
    const colIdx = (name) => rawHeaders.indexOf(name);
